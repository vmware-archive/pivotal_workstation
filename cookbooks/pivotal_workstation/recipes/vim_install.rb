...
execute "get vim source" do
  command 'curl "ftp://ftp.vim.org/pub/vim/unix/vim-#{vim_version}.tar.bz2" | bunzip2 | tar -xf - -C #{IVM_HOME}'
  user WS_USER
end

execute "install vim" do
  command "cd #{VIM_HOME} && ./configure --with-features=huge --enable-rubyinterp && make && make install"
  user WS_USER
end
